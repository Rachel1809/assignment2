...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            lora_A_result = self.lora_A[self.active_adapter](x)
            lora_dropout_result = self.lora_dropout[self.active_adapter](lora_A_result)
            lora_B_result = self.lora_B[self.active_adapter](lora_dropout_result)
            result += self.scaling[self.active_adapter] * lora_B_result ### YOUR CODE HERE ###
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
